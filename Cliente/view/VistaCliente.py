...
from Cliente.controller.cliente_controller import cliente_controller
...
class VistaCliente(QWidget):
    ...
    '''
    def update_ui_cliente(self):
        self.label_nome_cognome.setText(self.controller_utente.get_nome() + " " + self.cliente_controller.get_cognome())
        self.label_data_nascita.setText("Data nascita: {}".format(self.cliente_controller.get_data_nascita()))
        self.label_luogo_nascita.setText("Luogo nascita: {}".format(self.cliente_controller.get_luogo_nascita()))
        self.label_cf.setText("Codice fiscale: {}".format(self.cliente_controller.get_cf()))
        self.label_codice.setText("Codice: {}".format(self.cliente_controller.get_cod_utente()))
        self.label_indirizzo.setText("Indirizzo: {}".format(self.cliente_controller.get_indirizzo()))
        self.label_telefono.setText("Telefono: {}".format(self.cliente_controller.get_telefono()))
        self.label_ruolo.setText("Ruolo: {}".format(self.cliente_controller.get_ruolo()))
        self.label_stipendio.setText("Stipendio: {}".format(self.cliente_controller.get_stipendio()))
        self.label_data_inizio_contratto.setText("Data inizio contratto: {}".format(self.cliente_controller.get_data_inizio_contratto()))
        self.label_data_scadenza_contratto.setText("Data scadenza contratto: {}".format(self.cliente_controller.get_data_scadenza_contratto()))
        self.update_ui()
    ...
    def show_modifica_Cliente(self):
        self.vista_modifica_cliente = VistaModificaCliente(self.Cliente_selezionato, self.controller_cliente, self.update_ui_cliente())
    ...
    def closeEvent(self, event):
        if self.end1==True:
            reply = QMessageBox.question(self, 'Eliminare?',
                                         "Sicuro di voler eliminare il cliente selezionato?",
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)

            if reply == QMessageBox.Yes:
                if not type(event) == bool:   #SI
                    event.accept()
                    self.controller_lista_del_personale.elimina_cliente_by_codice(self.controller_cliente.get_cod_cliente(), self.lista_dinamica)
                    self.update_ui()
                else:
                    sys.exit()
            else:
